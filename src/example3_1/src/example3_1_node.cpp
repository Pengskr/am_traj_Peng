...
public:
    void visualize(const Trajectory &traj, const vector<Vector3d> &route, int id = 0)
    {
        ...
        if (id == 0)
        {
            ...
            accMarker.color.g = 0.0 / 255.0;
            accMarker.color.b = 0.0 / 255.0;
        }
        else if (id == 1)
        {
            accMarker.color.r = 255.0 / 255.0;
...
{
    ros::init(argc, argv, "example3_1_node");
    ...
    int M_max = 5;
    int groupSize = 100;
    std::vector<double> durs_0, durs_1, durs_2;
    durs_0.clear(); durs_1.clear(); durs_2.clear();
    ...
    // 打开四个输出文件（每种方法一个文件）
    std::string pkg_path = ros::package::getPath("example3_1");
    std::string result_dir = pkg_path + "/results/";

    std::ofstream csv_red(result_dir + "RED_analytic_TvsKT.csv");
    std::ofstream csv_blue(result_dir + "BLUE__TvsKT.csv");

    for (int M = 4; M < M_max && ok(); M++)    // 段数
    {
        for (double KT = 10; KT < 110 && ok(); KT += 10)
        {
            route.clear();
            for (int i = 0; i < 1; i++){
                ...
                // route.push_back(Vector3d(0.0, 0.0, 5.0));
                route.push_back(Vector3d(-16.0, 0.0, 5.0));
                route.push_back(Vector3d(-8.0, -8.0, 5.0));
                route.push_back(Vector3d(0.0, 0.0, 5.0));
                route.push_back(Vector3d(8.0, 8.0, 5.0));
                route.push_back(Vector3d(16.0, 0.0, 5.0));
            }
            ...
            std::cout << "---------------------------------------------------------------------------------------" << std::endl;
            std::cout << "Number of Segments: " << M << ", KT: " << KT << std::endl;
            amTrajOpt.wTime = KT;
            ...
            durs_0 = traj.getDurations();
            // 计算总时间
            double total_time = std::accumulate(durs_0.begin(), durs_0.end(), 0.0);
            // 数值安全性检查
            if (total_time <= 0.0) {
            ...
            for (size_t i = 0; i < durs_0.size(); ++i) {
                double ratio = durs_0[i] / total_time;
            ...
            durs_1 = traj.getDurations();
            // 计算总时间
            total_time = std::accumulate(durs_1.begin(), durs_1.end(), 0.0);
            // 数值安全性检查
            if (total_time <= 0.0) {
                std::cerr << "Error: total_time must be positive." << std::endl;
                return -1;
            ...
            // 打印比例
            std::cout << std::fixed << std::setprecision(6);
            for (size_t i = 0; i < durs_1.size(); ++i) {
                double ratio = durs_1[i] / total_time;
                std::cout << "      dur[" << i << "] / total = " << ratio << std::endl;
            }
            ...
            opt.optimize();
            tc1 = std::chrono::high_resolution_clock::now();
            mav_traj2am_traj(opt, durs_2, traj_GD);
            ...
            durs_2 = traj_GD.getDurations();
            // 计算总时间
            total_time = std::accumulate(durs_2.begin(), durs_2.end(), 0.0);
            // 数值安全性检查
            if (total_time <= 0.0) {
                std::cerr << "Error: total_time must be positive." << std::endl;
                return -1;
            ...
            // 打印比例
            std::cout << std::fixed << std::setprecision(6);
            for (size_t i = 0; i < durs_2.size(); ++i) {
                double ratio = durs_2[i] / total_time;
                std::cout << "      dur[" << i << "] / total = " << ratio << std::endl;
            }            
            ...
            // 将平均值写入 CSV
            csv_red   << KT << "," << t_lap_0 << "," << durs_0[0] << "," << durs_0[1] << "," << durs_0[2] << "," << durs_0[3] << "\n";
            csv_blue  << KT << "," << t_lap_1 << "," << durs_1[0] << "," << durs_1[1] << "," << durs_1[2] << "," << durs_1[3] << "\n";
        }
        csv_red.close();
        csv_blue.close();
    }
