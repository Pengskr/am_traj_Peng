...
public:
    ...
    Piece(double dur, CoefficientMat coeffs) : duration(dur)
    {
        ...
        {
            nCoeffMat.col(i) = coeffs.col(i) * t;   // 时间归一化系数
            t *= dur;
        }
    }
    ...
    inline bool checkMaxVelRate(double maxVelRate) const
    {
        ...
        if (getVel(0.0).squaredNorm() >= sqrMaxVelRate ||
            getVel(duration).squaredNorm() >= sqrMaxVelRate)    // 起点 或 终点的速度大于约束，返回false
        {
            return false;
        }
        else
        {
            ...
            Eigen::VectorXd coeff = RootFinder::polySqr(nVelCoeffMat.row(0)) +
                                    RootFinder::polySqr(nVelCoeffMat.row(1)) +
                                    RootFinder::polySqr(nVelCoeffMat.row(2));   // 系数卷积
            ...
            return RootFinder::countRoots(coeff, 0.0, 1.0) == 0;    //  使用 Sturm theory 判断根的数量
        }
    }
    ...
    inline bool checkMaxAccRate(double maxAccRate) const
    {
        ...
        if (getAcc(0.0).squaredNorm() >= sqrMaxAccRate ||
            getAcc(duration).squaredNorm() >= sqrMaxAccRate)    // 起点 或 终点的加速度大于约束，返回false
        {
            return false;
        }
        else
...
public:
    ...
    {
        int N = std::min(durs.size(), coeffMats.size());    // 轨迹段数 M
        for (int i = 0; i < N; i++)
        {
    ...
    //
    inline std::vector<CoefficientMat> getCoeffMats(bool normalized = false) const
    {
        std::vector<CoefficientMat> coeffMats;
        coeffMats.reserve(getPieceNum());
        for (int i = 0; i < getPieceNum(); i++)
        {
            coeffMats.push_back(pieces[i].getCoeffMat(normalized)); // 归一化系数矩阵的正确性未验证
        }
        return coeffMats;
    }
...
public:
    ...
    // Compute optimal coefficient matrices for all pieces: Md=b (LU decomp.)
            ...
            for (int j = 0; j < tryOut; j++)
            {
                ...
                for (int i = 0; i < N; i++)
                {
                    if (!traj[i].checkMaxAccRate(maxAccRate) ||
                        !traj[i].checkMaxVelRate(maxVelRate))   // 速度 或 加速度 不满足约束
                    {
    ...
    double evaluateObjective(const BoundaryCond &boundCond, double duration) const
    {
        ...
        return (wTime * duration +
                ...
                wJerk * RootFinder::polyVal(coeffsJerObjective, duration) * 3.0 / t5);  // 归一化时间能量 缩放
    }
...
public:
    ...
    void optimizeCoeffsConstrained(Trajectory &traj, int &idxPieceStuck) const
    {
        ...
        std::vector<CoefficientMat> targetCoeffsMats = optimizeCoeffs(posVec, durVec, velIni, accIni, velFin, accFin);
    ...
    // 在系数向量或者导数向量固定的情况下优化T，可以直接解析求解（待修改）
    void optimizeDurations(Trajectory &traj, bool constrained = true) const
    {
        ...
        for (int i = 0; i < N; i++)
        {
            ...
            // std::cout << "Num of stationaries:" << stationaries.size() << std::endl;
